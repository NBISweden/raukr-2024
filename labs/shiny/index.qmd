...
webr::install(c("ggplot2", "bslib"))

library(bslib)
library(ggplot2)

pc <- prcomp(iris[,c("Sepal.Length", "Petal.Length", "Sepal.Width", "Petal.Width")], center = TRUE, scale. = TRUE)
dfr <- cbind(iris,as.data.frame(pc$x))
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    ...
    ggplot(dfr, aes(x = !!sym(x), y = !!sym(y), col = Species)) +
      geom_point() +
      theme_bw() + 
